namespace QxRoom.Common.Mail
{
    ...
    public class SmtpServerHelper
    {
        ...
        private bool SendEmail(string smtpServer, int port, bool ESmtp, string username, string password, MailMessage mailMessage)
        {
            ...
            {
                strArray[i] = "RCPT TO:<" + ((string) mailMessage.Recipients[i]) + ">" + this.CRLF;
            }
            ...
            string str3 = str;
            str = str3 + "To:=?" + mailMessage.Charset.ToUpper() + "?B?" + this.Base64Encode((string) mailMessage.Recipients[0]) + "?=<" + ((string) mailMessage.Recipients[0]) + ">" + this.CRLF + (((mailMessage.Subject == string.Empty) || (mailMessage.Subject == null)) ? "Subject:" : ((mailMessage.Charset == "") ? ("Subject:" + mailMessage.Subject) : ("Subject:=?" + mailMessage.Charset.ToUpper() + "?B?" + this.Base64Encode(mailMessage.Subject) + "?="))) + this.CRLF + "X-Priority:" + priorityString + this.CRLF + "X-MSMail-Priority:" + priorityString + this.CRLF + "Importance:" + priorityString + this.CRLF + "X-Mailer: Lion.Web.Mail.SmtpMail Pubclass [cn]" + this.CRLF + "MIME-Version: 1.0" + this.CRLF;
            if (mailMessage.Attachments.Count != 0)
            {
                string str4 = str + "Content-Type: multipart/mixed;" + this.CRLF;
                str = str4 + " boundary=\"=====" + (flag ? "001_Dragon520636771063_" : "001_Dragon303406132050_") + "=====\"" + this.CRLF + this.CRLF;
            }
            if (flag)
            {
                if (mailMessage.Attachments.Count == 0)
            ...
            if (mailMessage.Attachments.Count != 0)
            {
                ...
                {
                    string filePath = mailMessage.Attachments[j];
                    string str6 = str;
                    string str7 = str6 + "--=====" + (flag ? "001_Dragon520636771063_" : "001_Dragon303406132050_") + "=====" + this.CRLF + "Content-Type: text/plain;" + this.CRLF;
                    string str8 = str7 + " name=\"=?" + mailMessage.Charset.ToUpper() + "?B?" + this.Base64Encode(filePath.Substring(filePath.LastIndexOf(@"\") + 1)) + "?=\"" + this.CRLF + "Content-Transfer-Encoding: base64" + this.CRLF + "Content-Disposition: attachment;" + this.CRLF;
                    str = str8 + " filename=\"=?" + mailMessage.Charset.ToUpper() + "?B?" + this.Base64Encode(filePath.Substring(filePath.LastIndexOf(@"\") + 1)) + "?=\"" + this.CRLF + this.CRLF + this.GetStream(filePath) + this.CRLF + this.CRLF;
                }
                string str9 = str;
                str = str9 + "--=====" + (flag ? "001_Dragon520636771063_" : "001_Dragon303406132050_") + "=====--" + this.CRLF + this.CRLF;
            }
