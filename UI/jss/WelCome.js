...
function Bind()
{
    ...
    for(var i=0;i<imgDomLength;i++)
    {
        ...
        try{
            ...
            var returnStr = Get_XmlHttp("/Work/XMGL/publicBll.ashx?loop=Welcome&url="+encodeURIComponent(url)+"&userid="+userid.value);
            imgDom[i].id=imgDom[i].title=returnStr.split('|')[1];
            if(i==1||i==3||i==5||i==6)
        ...
        function OpenWindow(objNode)
        {
            ...
            if(objtarget == "mainFrame")
            {
                ...
                window.parent.addTabPanel(objNode.id, objNode.id, objtitle, objNode.value);
            }else
            {
    ...
    //获取页面大小初始值
        function SetWH() {
            //var w = document.body.scrollWidth; //clientWidth, offsetWidth
            //var h = document.body.scrollHeight; //clientHeight, offsetHeight 
