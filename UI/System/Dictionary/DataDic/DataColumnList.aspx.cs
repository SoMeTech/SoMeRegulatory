...
public class SystemSetup_Dictionary_DataDic_DataColumnList : MainPageClass, IRequiresSessionState
{
    private DataColumnDal cod;
    ...
    public void Buttons(string ibtid)
    {
        switch (ibtid)
        {
            case "ibtcontrol_ibtadd":
                PageShowText.OpenPage("DataColumnAdd.aspx?strTitle=" + base.Server.UrlEncode(this.txttitle.Value.Trim()) + "&reload=MainAddUpdate", 800, 500, this.Page);
                return;

            case "ibtcontrol_ibtupdate":
                if ((this.gvResult.SelectedIndex >= 0) && (this.gvResult.Rows.Count >= this.gvResult.SelectedIndex))
                {
                    PageShowText.OpenPage("DataColumnAdd.aspx?PK=" + ((Label)this.gvResult.Rows[this.gvResult.SelectedIndex].FindControl("labPk")).Text.Trim() + "&strTitle=" + base.Server.UrlEncode(this.txttitle.Value.Trim()), 800, 500, this.Page);
                    return;
                }
    ...
    protected void gvResult_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        else
        {
            ...
            if (((str = e.CommandName) != null) && (str != "Select"))
            {
                ...
                {
                    num = int.Parse(e.CommandArgument.ToString());
                    PageShowText.OpenPage("DataColumnAdd.aspx?PK=" + this.gvResult.DataKeys[num].Value.ToString() + "&strTitle=" + base.Server.UrlEncode(this.txttitle.Value.Trim()), 800, 500, this.Page);
                }
            }
        }
    }
    ...
    {
        PageUsuClass.PageChangControl(nPageIndex, "DataColumnList", this.Page);
    }
    ...
    {
        PageUsuClass.SearchControl(strselect, strcloname, strsql, this.pageind, "DataColumnList", this.Page);
    }
    ...
    public void ShowData(string str)
    {
        try
        {
            ...
            DataSet set = this.pagesize.pagesize(DataColumnDal.GetViewGOV_TC_VIEW_DATADICColumns(), "GOV_TC_VIEW_DATADIC", str, "PK", "", this.Master.PageIndex, this.Master.PageSize, out this.count);
