function BindTxtSelect(txtobj,data,tableObj,columnName)
{
    if(data==null)
    {
//        var table = document.getElementById(tableID);
        var columnIndex=0;
        for(columnIndex=0;columnIndex<tableObj.rows[0].cells.length;columnIndex++)
        {
            if(trim(tableObj.rows[0].cells[columnIndex].innerText)==trim(columnName))
                break;
        }
        if(columnIndex==tableObj.rows[0].cells.length)
            return;
        
        var dataList = new Array();
        for(var i=1;i<tableObj.rows.length;i++)
        {
            if(tableObj.rows[i].className=="HeaderRow")
                continue;
                
            var tableObjInnerText=tableObj.rows[i].cells[columnIndex].innerText;
            if(trim(tableObjInnerText)=="")
                continue;
            dataList.push(tableObjInnerText);
        }
        data = dataList.distinct();
//        var dataStr = dataList.join("|");
//        alert(dataStr);
//        data=dataStr.split('|');
//        alert(data);
    }
	var txtSelect=(autoComplete={
		pop_len:10,
		pop_cn:'autoDis',
		hover_cn:'cur',
		source: null,
		...
		insert:function(self){
			...
			for(var i=0,leng=this.source.length;i<leng;i++){         //判断input的数据是否与数据源里的数据一致
				...
				else if(!!val&&val.length<=this.source[i].length&& this.source[i].substr(0,val.length) == val){
					bak.push(this.source[i]);
				}
			}
			...
			with(this.pop){
				style.cssText = 'z-index:99999;width:'+self.offsetWidth+'px;'+'position:absolute;left:'+left+'px;top:'+top+'px;display:none;';
	...
	txtSelect.init(data).bind(txtobj);
}
