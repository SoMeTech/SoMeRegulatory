<%@ page language="C#" autoeventwireup="true" inherits="list" masterpagefile="~/Master/One.master" CodeBehind="list.aspx.cs" enableEventValidation="false" stylesheettheme="Default" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    ...
    <yyc:SmartGridView ID="gvResult" runat="server" AutoGenerateColumns="False" MouseOverCssClass="OverRow"
        ContextMenuCssClass="RightMenu" Width="100%" DataKeyNames="PD_PROJECT_CODE" AllowSorting="true"
       BoundRowClickCommandName="Select" BoundRowDoubleClickCommandName="Two"  TimeSpan="1000" ondblclick="gvResultClientClick()">
        <Columns>
            ...
            <asp:TemplateField HeaderText="标题" SortExpression="PD_PROJECT_SUBJECTS" ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="Left">
                <ItemTemplate>
                    <asp:Label ID="labPD_PROJECT_SUBJECTS" runat="server" Text='<%#Eval("PD_PROJECT_SUBJECTS") %>'></asp:Label>
                </ItemTemplate>
            </asp:TemplateField>
            <asp:TemplateField HeaderText="类别" SortExpression="TrueName"  ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="Left">
                <ItemTemplate>
                    <asp:Label ID="labPD_PROJECT_TYPE" runat="server" Text='<%#getComfirm(Eval("PD_PROJECT_TYPE")) %>'></asp:Label>       <%-- '<%#Eval("PD_PROJECT_TYPE").ToString()=="1"?"工作动态":"法律法规" %>'--%>
                </ItemTemplate>
            </asp:TemplateField>
            <asp:TemplateField HeaderText="发布时间" SortExpression="PD_PROJECT_SYSDATE"  ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="Left">
                <ItemTemplate>
                    <asp:Label ID="labPD_PROJECT_SYSDATE" runat="server" Text='<%#Eval("PD_PROJECT_SYSDATE") %>'></asp:Label>
                </ItemTemplate>
            </asp:TemplateField>        
