...
public class Work_projectGL_ssProjectGKGS_proJSGKGSMX : Page, IRequiresSessionState
{
    ...
    protected HtmlInputText txt_PD_PROJECT_CONTENT;
    protected TextBox txt_PD_PROJECT_FILENO_LX;
    protected TextBox txt_PD_PROJECT_FZR;
    protected HtmlInputText txt_PD_PROJECT_MONEY_CZ_BJ;
    protected HtmlInputText txt_PD_PROJECT_MONEY_CZ_SJ;
    protected HtmlInputText txt_PD_PROJECT_MONEY_CZ_TOTAL;
    protected HtmlInputText txt_PD_PROJECT_MONEY_QT;
    protected HtmlInputText txt_PD_PROJECT_MONEY_TOTAL;
    protected HtmlInputText txt_PD_PROJECT_MONEY_ZC;
    protected HtmlInputText txt_PD_PROJECT_SSFW;
    protected TextBox txt_PD_PROJECT_SYRS;
    protected TextBox txt_PD_PROJECT_YEARS;
    ...
    private void AddImgMethod(PD_PROJECT_GKGS_Model model)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(str))
            {
                ...
                str = "UserImages/" + str3;
                DbHelperOra.ExecuteSql(string.Concat(new object[] { "insert into t_photos (fkid, photoname, photopath, remarks, sortid) values  ( 'ssGKGS_", model.AUTO_NO, "', '", str3, "', '", str, "', '", str3, "', 1)" }));
            }
        }
        catch
        {
    ...
    public void BindImg()
    {
        ...
        try
        {
            ...
            DataSet set = DbHelperOra.Query("select * from t_photos where fkid='ssGKGS_" + str_MXID + "'");
    ...
    public void Buttons(string ibtid)
    {
        ...
        switch (ibtid)
        {
            case "ibtcontrol_ibtadd":
                ...
                base.Response.Redirect(this.Page.Request.Url.LocalPath + "?doType=add&UpdatePK=" + str + "&strTitle=" + base.Request["strTitle"]);
                return;

            case "ibtcontrol_ibtupdate":
                this.Update();
                str = base.Request.Params["UpdatePK"].ToString();
                base.Response.Redirect(this.Page.Request.Url.LocalPath + "?doType=update&UpdatePK=" + str + "&strTitle=" + base.Request["strTitle"]);
    ...
    public static int DelImg(string value1)
    {
        ...
        {
            return DbHelperOra.ExecuteSql("delete from t_photos   where fkid='ssGKGS_" + str_MXID + "' and PHOTONAME='" + value1.Trim() + "'");
        }
        return 0;
    }
    ...
    [WebMethod]
    public static string GetGsxs()
    {
        string sQLString = "select distinct pd_gs_style from pd_project_gkgs where  pd_gs_style is not null";
        ...
        {
            builder.Append(row["pd_gs_style"].ToString() + "|");
        }
    ...
    [WebMethod]
    public static string LookImag(string value)
    {
        string.IsNullOrEmpty(str_MXID);
        return "";
    }
                ...
                if ((base.Request["UpdatePK"] != null) && PublicDal.PageValidate.IsDecimal(base.Request["UpdatePK"]))
                {
                    ...
                    else
                    {
                        if (base.Request["doType"] == "look")
                        {
                            try
                            {
                                str = base.Request["GS_ID"].ToString();
                            }
                            catch
                            {
                            }
                        ...
                        else if (base.Request["doType"] == "looklist")
                        {
                            ...
                            {
                                str = this.gvResult.DataKeys[this.gvResult.Rows.Count - 1].Value.ToString();
                            }
                            else
                            {
                                this.ShowInfo_Project(base.Request["UpdatePK"].ToString());
        ...
        if (base.Request["doType"] == "add")
        {
            if (this.txtPD_PROJECT_CODE.Value != null)
            {
                ...
                try
                {
                    ...
                    this.ShowInfo_Project(model.PD_PROJECT_CODE);
                }
                catch
                {
                    Const.DoSuccessNoClose("添加失败", this.Page.Request.Url.LocalPath + "?doType=look&UpdatePK=" + base.Request["UpdatePK"].ToString(), this.Page);
                }
            }
        }
    ...
    private void ShowInfo(int AUTO_NO_GS)
    {
        ...
        this.txtPD_GS_TYPE.SelectedValue = model.PD_GS_TYPE.ToString();
        this.txtPD_GS_STYLE.Value = model.PD_GS_STYLE.ToString();
        this.txtPD_GS_ZHUTI.Value = model.PD_GS_ZHUTI.ToString();
        if (model.PD_GS_DATE.HasValue)
        {
            this.txtPD_GS_DATE.Value = model.PD_GS_DATE.Value.ToString("yyyy-MM-dd");
        }
        this.txtPD_GS_ADDR.Value = model.PD_GS_ADDR.ToString();
        this.txtPD_GS_DETAIL.Value = model.PD_GS_DETAIL.ToString();
        if (model.PD_GS_DATE_END.HasValue)
        {
            this.txtPD_GS_DATE_END.Value = model.PD_GS_DATE_END.Value.ToString("yyyy-MM-dd");
        }
        ...
        if (((model != null) && (model.PD_GS_FILENAME_SYSTEM != null)) && (model.PD_GS_FILENAME_SYSTEM.Trim() != ""))
        {
            ...
            row["FILE_NAME"] = model.PD_GS_FILENAME_SYSTEM;
    ...
    private void ShowInfo_Project(string PD_PROJECT_CODE)
    {
        ...
        this.txtPD_YEAR.Value = model.PD_YEAR.ToString();
        this.txtPD_PROJECT_CODE.Value = model.PD_PROJECT_CODE.ToString();
        this.txtPD_PROJECT_NAME.Text = model.PD_PROJECT_NAME.ToString();
        this.txt_PD_PROJECT_FILENO_LX.Text = model.PD_PROJECT_FILENO_LX.ToString();
        this.txtPD_PROJECT_BZNUM.Value = model.PD_PROJECT_OPEN_BODY.ToString();
        this.txt_PD_PROJECT_FZR.Text = model.PD_PROJECT_FZR.ToString();
        this.txt_PD_PROJECT_SYRS.Text = model.PD_PROJECT_SYRS.ToString();
        this.txt_PD_PROJECT_SSFW.Value = model.PD_PROJECT_SSFW.ToString();
        this.txt_PD_PROJECT_YEARS.Text = model.PD_PROJECT_YEARS.ToString();
        this.txt_PD_PROJECT_CONTENT.Value = model.PD_PROJECT_CONTENT.ToString();
        this.txt_PD_PROJECT_MONEY_TOTAL.Value = model.PD_PROJECT_MONEY_TOTAL.ToString();
        this.txt_PD_PROJECT_MONEY_CZ_TOTAL.Value = model.PD_PROJECT_MONEY_CZ_TOTAL.ToString();
        this.txt_PD_PROJECT_MONEY_CZ_SJ.Value = model.PD_PROJECT_MONEY_CZ_SJ.ToString();
        this.txt_PD_PROJECT_MONEY_CZ_BJ.Value = model.PD_PROJECT_MONEY_CZ_BJ.ToString();
        this.txt_PD_PROJECT_MONEY_ZC.Value = model.PD_PROJECT_MONEY_ZC.ToString();
        this.txt_PD_PROJECT_MONEY_QT.Value = model.PD_PROJECT_MONEY_QT.ToString();
        DbHelperOra.Query("select * from pd_project_contract where pd_project_code=" + PD_PROJECT_CODE);
        string sQLString = "";
        int num = 0;
        try
        {
            num = Convert.ToInt32(this.gvResult.DataKeys[Work_projectGL_ssProjectGKGS_proJSGKGSMX.num].Value.ToString());
        ...
        if ((set != null) && (set.Tables[0].Rows.Count > 0))
        {
            ...
            str_MXID = this.lblAUTO_NO.Text.ToString();
            ...
            if (PublicDal.PageValidate.IsDateTime(set.Tables[0].Rows[0]["PD_GS_DATE"]))
            {
                this.txtPD_GS_DATE.Value = DateTime.Parse(set.Tables[0].Rows[0]["PD_GS_DATE"].ToString()).ToString("yyyy-MM-dd");
            }
            ...
            if (PublicDal.PageValidate.IsDateTime(set.Tables[0].Rows[0]["PD_GS_DATE_END"]))
            {
                this.txtPD_GS_DATE_END.Value = DateTime.Parse(set.Tables[0].Rows[0]["PD_GS_DATE_END"].ToString()).ToString("yyyy-MM-dd");
            }
            DataSet ds = new DataSet();
            ds.Tables.Add();
            ds.Tables[0].Columns.Add("AUTO_NO");
    ...
    public static int UpdateImgDscript(string value1, string value2, string value3)
    {
        ...
        if (!string.IsNullOrEmpty(str_MXID))
        {
            return DbHelperOra.ExecuteSql("update t_photos set remarks='" + value1 + "' where fkid='ssGKGS_" + str_MXID + "' and photoname='" + value3 + "'");
        }
        return 0;
    }
