...
public class Work_projectGL_HeTongBG_HeTongBGList : Page, IRequiresSessionState
{
    ...
    public void Buttons(string ibtid)
    {
        ...
        switch (ibtid)
        {
            case "ibtcontrol_ibtadd":
                PageShowText.OpenPage("HeTongBGMx.aspx", 0x556, 0x300, this.Page);
                break;
            ...
            case "ibtcontrol_ibtdelete":
                {
                    ...
                    {
                        this.DataDelete(long.Parse(text));
                    }
                    return;
                }
            case "ibtcontrol_ibtdo":
                ...
                PageShowText.OpenPage("HeTongBGMx.aspx?UpdatePK=" + this.gvResult.DataKeys[selectedIndex].Value.ToString() + "&bgType=bgList&strTitle=" + base.Server.UrlEncode(this.txttitle.Value.Trim()), 0x556, 0x300, this.Page);
                return;
    ...
    private void DataDelete(long AUTO_NO)
    {
        try
        {
            new PD_CONTRACT_CHANGE_Bll().Delete(AUTO_NO);
            if (this.Master.PageIndex > 1)
            {
                this.pageind = this.Master.PageIndex;
            }
    ...
    protected void gvResult_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        else
        {
            ...
            if (commandName != null)
            {
                ...
                {
                    num = int.Parse(e.CommandArgument.ToString());
                    PageShowText.OpenPage("HeTongBGMx.aspx?UpdatePK=" + this.gvResult.DataKeys[num].Value.ToString() + "&strTitle=" + base.Server.UrlEncode(this.txttitle.Value.Trim()), 0x556, 0x300, this.Page);
                }
            }
        }
    }
    ...
    public void ShowData(string str)
    {
        try
        {
            ...
            DataSet set = this.pagesize.pagesize(this.getColumns(), "view_contract_change_list", str, "pd_project_code", " order by pd_project_code,auto_no desc ", this.Master.PageIndex, this.Master.PageSize, out this.count);
