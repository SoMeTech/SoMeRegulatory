...
public class Work_GL_SbList : Page, IRequiresSessionState
{
    ...
    public void Buttons(string ibtid)
    {
        ...
        switch (ibtid)
        {
            case "ibtcontrol_ibtadd":
                PageShowText.OpenPage("SbMx.aspx?strTitle=" + base.Server.UrlEncode(this.txttitle.Value.Trim()), null, null, this.Page);
                return;
            ...
            case "ibtcontrol_ibtdo":
                if ((selectedIndex >= 0) && (this.gvResult.Rows.Count >= selectedIndex))
                {
                    ...
                    {
                        PageShowText.OpenPage("SbMx.aspx?UpdatePK=" + this.gvResult.DataKeys[selectedIndex].Value.ToString() + "&strTitle=" + base.Server.UrlEncode(this.txttitle.Value.Trim()), null, null, this.Page);
                    }
                    return;
                }
    ...
    private void DeleteObject(string p)
    {
        ...
        {
            new TB_PROJECT_Bll().Delete(p);
            new PD_PROJECT_ATTACH_PF_Bll().Delete(p);
            new PD_CONTRACT_CHANGE_Bll().DeletePROJECT(p);
            new PD_PROJECT_CONTRACT_Bll().DeletePROJECT(p);
            new PD_PROJECT_ATTACH_SB_Bll().DeleteList("'" + p + "'");
            new PD_PROJECT_INSPECTION_Bll().DeleteList("'" + p + "'");
            new PD_PROJECT_JGYS_Bll().Delete(p);
            new PD_PROJECT_ATTACH_YS_Bll().Delete(p);
            new PD_PROJECT_APPRAISE_Bll().DeletePROJECT(p);
            if (this.Master.PageIndex > 1)
            {
                this.pageind = this.Master.PageIndex;
            }
    ...
    protected void gvResult_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        else
        {
            ...
            if (commandName != null)
            {
                ...
                {
                    num = int.Parse(e.CommandArgument.ToString());
                    PageShowText.OpenPage("SbMx.aspx?UpdatePK=" + this.gvResult.DataKeys[num].Value.ToString() + "&strTitle=" + base.Server.UrlEncode(this.txttitle.Value.Trim()), 0x556, 0x300, this.Page);
                }
            }
        }
    }
    ...
    public void ShowData(string str)
    {
        try
        {
            ...
            DataSet ds = this.pagesize.pagesize("*", "view_pd_project_findlist", str, "pd_project_code", "", this.Master.PageIndex, this.Master.PageSize, out this.count);
