...
public class Work_projectGL_ssProjectCCXC_ssXCMx : Page, IRequiresSessionState
{
    private DB_OPT dbo;
    ...
    protected HtmlInputText txtPD_MONITOR_PROCEED_WCL;
    protected HtmlInputText txtPD_MONITOR_TOTAL_MONEY_PAY;
    protected HtmlInputText txtPD_MONITOR_TOTAL_MONEY_WCL;
    protected HtmlInputText txtPD_PROJECT_CODE;
    protected HtmlInputText txtPD_PROJECT_Name;
    protected HtmlInputText txtPD_PROJECT_TOTAL_MONEY;
    ...
    private void AddImgMethod(PD_PROJECT_INSPECTION_Model model)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(str))
            {
                ...
                str = "UserImages/" + str3;
                DbHelperOra.ExecuteSql("insert into t_photos (fkid, photoname, photopath, remarks, sortid) values  ( 'ssXCMX_" + model.AUTO_NO + "', '" + str3 + "', '" + str + "', '" + str3 + "', 1)");
            }
        }
        catch
        {
    ...
    public void BindImg()
    {
        ...
        try
        {
            ...
            DataSet set = DbHelperOra.Query("select * from t_photos where fkid='ssXCMX_" + str_MXID + "'");
    ...
    public void Buttons(string ibtid)
    {
        switch (ibtid)
        {
            case "ibtcontrol_ibtprintnote":
                PageShowText.OpenPage(Public.RelativelyPath("Report/jsxGTable2.aspx") + "?UpdatePK=" + base.Request["UpdatePK"].Trim(), null, null, this.Page, true);
            ...
            case "ibtcontrol_ibtadd":
                {
                    ...
                    str_MXID = "";
                    base.Response.Redirect("ssXCMx.aspx?CCXCDoType=add&UpdatePK=" + str + "&strTitle=" + base.Request["strTitle"].ToString());
                    return;
                }
    ...
    public static int DelImg(string value1)
    {
        ...
        {
            return DbHelperOra.ExecuteSql("delete from t_photos   where fkid='ssXCMX_" + str_MXID + "' and PHOTONAME='" + value1.Trim() + "'");
        }
        return 0;
    }
        ...
        else if (base.Request["CCXCDoType"] != "add")
        {
            ...
            if (commandName != null)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        base.Response.Redirect("ssXCMx.aspx?CCXCDoType=look&UpdatePK=" + base.Request["UpdatePK"].ToString() + "&MXID=" + str + "&strTitle=" + base.Request["strTitle"].ToString());
                    }
                    catch
                    {
                        base.Response.Redirect("ssXCMx.aspx?CCXCDoType=look&UpdatePK=" + base.Request["UpdatePK"].ToString() + "&MXID=" + str + "&strTitle=项目建设资金巡查");
                    }
                }
            }
        }
                ...
                if ((base.Request["UpdatePK"] != null) && PublicDal.PageValidate.IsDecimal(base.Request["UpdatePK"]))
                {
                    ...
                    if (this.gvResult.Rows.Count > 0)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                this.ShowInfo_New(base.Request["UpdatePK"].ToString());
                                this.Session["CCXCDoType"] = "add";
                                model.IfUpdate = false;
                                model.IfDelete = false;
                                this.gvResult.Enabled = false;
    ...
    public void ShowData(string str)
    {
        try
        {
            ...
            str = "select " + this.getColumns() + " from  view_inspection_list " + str;
    ...
    private void ShowInfo_New(string PD_Project_Code)
    {
        TB_PROJECT_Model model = new TB_PROJECT_Bll().GetModel(PD_Project_Code);
        this.txtPD_PROJECT_CODE.Value = model.PD_PROJECT_CODE;
        this.txtPD_PROJECT_Name.Value = model.PD_PROJECT_NAME;
        this.ddlPD_YEAR.SelectedValue = model.PD_YEAR.Value.ToString();
    }
    ...
    public static int UpdateImgDscript(string value1, string value2, string value3)
    {
        ...
        if (!string.IsNullOrEmpty(str_MXID))
        {
            return DbHelperOra.ExecuteSql("update t_photos set remarks='" + value1 + "' where fkid='ssXCMX_" + str_MXID + "'and photoname='" + value3 + "'");
        }
        return 0;
    }
