...
public partial class Work_BZ_GKGS_BZGKGSList : Page, IRequiresSessionState
{
        ...
        switch (ibtid)
        {
            case "ibtcontrol_ibtlook":
                if ((selectedIndex >= 0) && (this.gvResult.Rows.Count >= selectedIndex))
                {
                    ...
                    PageShowText.OpenPage("BZGKGSMX.aspx?doType=looklist&UpdatePK=" + this.gvResult.DataKeys[selectedIndex].Value.ToString() + "&strTitle=补助性资金公开公示", 0x556, 0x300, this.Page);
                    return;
                }
    ...
    protected void gvResult_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        else
        {
            ...
            if (commandName != null)
            {
                ...
                {
                    num = int.Parse(e.CommandArgument.ToString());
                    PageShowText.OpenPage("BZGKGSMX.aspx?UpdatePK=" + this.gvResult.DataKeys[num].Value.ToString() + "&strTitle=" + base.Server.UrlEncode(this.txttitle.Value.Trim()), 0x556, 0x300, this.Page);
                }
            }
        }
    }
    ...
    public void ShowData(string str)
    {
        try
        {
            ...
            if (str != "")
            {
                str = " where " + str + " and  pd_found_xz='02'  and   nvl(pd_project_isbxk,0)=0 " + companyWhere;
            }
            else
            {
