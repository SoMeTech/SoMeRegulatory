...
public partial class Work_BZ_projBzList : Page, IRequiresSessionState
{
    ...
    public void Buttons(string ibtid)
    {
        ...
        switch (ibtid)
        {
            case "ibtcontrol_ibtadd":
                PageShowText.OpenPage("BzMx.aspx?ccxc=1&strTitle=补助性项目登记", 0x556, 0x300, this.Page);
                return;

            case "ibtcontrol_ibtdo":
                if ((selectedIndex >= 0) && (this.gvResult.Rows.Count >= selectedIndex))
                {
                    PageShowText.OpenPage("BzMx.aspx?ccxc=1&UpdatePK=" + this.gvResult.DataKeys[selectedIndex].Value.ToString() + "&strTitle=补助性项目登记", 0x556, 0x300, this.Page);
                    return;
                }
    ...
    private void DeleteObject(string p)
    {
        ...
        {
            new TB_PROJECT_BZ_Bll().Delete(p);
            new PD_PROJECT_INSPECTION_Bll().Delete(p, null);
            new PD_PROJECT_ATTACH_BZ_Bll().Delete(p);
            PageShowText.Refurbish("删除成功！", this.Page);
        }
        else
        {
            PageShowText.Refurbish("单据已进入业务流程，需删除请追回单据后再进行删除。", this.Page);
        }
    }
    ...
    protected void gvResult_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        else
        {
            ...
            if (commandName != null)
            {
                ...
                {
                    num = int.Parse(e.CommandArgument.ToString());
                    PageShowText.OpenPage("BzMx.aspx?ccxc=1&UpdatePK=" + this.gvResult.DataKeys[num].Value.ToString() + "&strTitle=" + base.Server.UrlEncode(this.txttitle.Value.Trim()), 0x556, 0x300, this.Page);
                }
            }
        }
    }
