...
public class Work_BZ_projBtFFList : Page, IRequiresSessionState
{
    ...
    public void Buttons(string ibtid)
    {
        ...
        switch (ibtid)
        {
            case "ibtcontrol_ibtadd":
                PageShowText.OpenPage("BtFFMx.aspx", 0x556, 0x300, this.Page);
                return;

            case "ibtcontrol_ibtdo":
                if ((selectedIndex >= 0) && (this.gvResult.Rows.Count >= selectedIndex))
                {
                    PageShowText.OpenPage("BtFFMx.aspx?UpdatePK=" + this.gvResult.DataKeys[selectedIndex].Value.ToString() + "&strTitle=" + base.Server.UrlEncode(this.txttitle.Value.Trim()), 0x556, 0x300, this.Page);
                    return;
                }
    ...
    private void DataDelete(string strPK)
    {
        try
        {
            new PD_PROJECT_BZFFLIST_Bll().Delete(strPK);
            if (this.Master.PageIndex > 1)
            {
                this.pageind = this.Master.PageIndex;
            }
    ...
    protected void gvResult_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        else
        {
            ...
            if (commandName != null)
            {
                ...
                {
                    num = int.Parse(e.CommandArgument.ToString());
                    PageShowText.OpenPage("projBtMx.aspx?UpdatePK=" + this.gvResult.DataKeys[num].Value.ToString() + "&strTitle=" + base.Server.UrlEncode(this.txttitle.Value.Trim()), 0x556, 0x300, this.Page);
                }
            }
        }
    }
    ...
    {
        PageUsuClass.PageChangControl(nPageIndex, "PD_PROJECT_BZFFLIST", this.Page);
    }
    ...
    {
        PageUsuClass.SearchControl(strselect, strcloname, strsql, this.pageind, "PD_PROJECT_BZFFLIST", this.Page);
    }
    ...
    public void ShowData(string str)
    {
        try
        {
            ...
            if (str != "")
            {
                str = " where " + str + " and  1=1 ";
            }
            else
            {
                str = "where  1=1 ";
            }
            DataSet set = this.pagesize.pagesize("*", "PD_PROJECT_BZFFLIST", str, "auto_no", " order by auto_no desc ", this.Master.PageIndex, this.Master.PageSize, out this.count);
