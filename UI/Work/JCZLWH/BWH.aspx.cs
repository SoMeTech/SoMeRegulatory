...
public class Work_JCZLWH_BWH : Page, IRequiresSessionState
{
    ...
    private void doEdit()
    {
        ...
        {
            string str = this.gvResult.Rows[i].Cells[2].Text.ToString();
            TextBox box = (TextBox)this.gvResult.Rows[i].Cells[4].FindControl("RowSelectorZW");
            string str2 = box.Text.ToString();
            string str3 = "0";
            CheckBox box2 = (CheckBox)this.gvResult.Rows[i].Cells[6].FindControl("RowSelector");
            if (box2.Checked)
            {
                str3 = "1";
            }
            string str4 = "0";
            CheckBox box3 = (CheckBox)this.gvResult.Rows[i].Cells[8].FindControl("RowSelector2");
            if (box3.Checked)
            {
                str4 = "1";
            }
            DbHelperOra.ExecuteSql("UPDATE DB_BZD  set nameZW='" + str2 + "',isJCB='" + str3 + "', isYWB  ='" + str4 + "' where tabName='" + str + "'");
        }
    }
    ...
    protected void gvResult_RowDataBound()
    {
        for (int i = 0; i < this.gvResult.Rows.Count; i++)
        {
            CheckBox box1 = (CheckBox)this.gvResult.Rows[i].Cells[1].FindControl("RowSelectorEdit");
            TextBox box = (TextBox)this.gvResult.Rows[i].Cells[4].FindControl("RowSelectorZW");
            CheckBox box2 = (CheckBox)this.gvResult.Rows[i].Cells[6].FindControl("RowSelector");
            CheckBox box3 = (CheckBox)this.gvResult.Rows[i].Cells[8].FindControl("RowSelector2");
            string str = this.gvResult.Rows[i].Cells[3].Text.ToString();
            box.Text = str;
            if (this.gvResult.Rows[i].Cells[5].Text.ToString() == "1")
            {
                box2.Checked = true;
            }
            else
            ...
            if (this.gvResult.Rows[i].Cells[7].Text.ToString() == "1")
            {
                box3.Checked = true;
            }
            else
        ...
        if (this.Session["user"] != null)
        {
            ...
            if (!this.Page.IsPostBack)
            {
                try
                {
                    ...
                    sQLStringList.Add("insert into DB_BZD (TABNAME,nameZW,ISJCB,ISYWB) select Table_name,Table_name,'0','0' from User_Tab_Comments where table_type='TABLE'  and Table_name not in (select TABNAME from DB_BZD)");
                    DbHelperOra.ExecuteSqlTran(sQLStringList);
                }
                catch (Exception exception)
                {
                    PageShowText.ShowMessage("表字典更新失败" + exception, this.Page);
                }
                string userName = ((UserModel)this.Session["User"]).UserName;
                string power = ((UserModel)this.Session["User"]).Power;
                ...
                catch (Exception exception2)
                {
                    this.el = new ExceptionLog.ExceptionLog();
                    this.el.ErrClassName = base.GetType().ToString();
                    this.el.ErrMessage = exception2.Message.ToString();
    ...
    {
        string sQLString = "select * from DB_BZD";
        DataTable table = DbHelperOra.Query(sQLString).Tables[0];
        this.gvResult.DataSource = table;
        this.gvResult.DataBind();
        this.gvResult.Columns[1].HeaderStyle.CssClass = "hidden";
        this.gvResult.Columns[1].ItemStyle.CssClass = "hidden";
        this.gvResult.Columns[1].FooterStyle.CssClass = "hidden";
        this.gvResult.Columns[3].HeaderStyle.CssClass = "hidden";
        this.gvResult.Columns[3].ItemStyle.CssClass = "hidden";
        this.gvResult.Columns[3].FooterStyle.CssClass = "hidden";
        this.gvResult.Columns[5].HeaderStyle.CssClass = "hidden";
        this.gvResult.Columns[5].ItemStyle.CssClass = "hidden";
        this.gvResult.Columns[5].FooterStyle.CssClass = "hidden";
        this.gvResult.Columns[7].HeaderStyle.CssClass = "hidden";
        this.gvResult.Columns[7].ItemStyle.CssClass = "hidden";
        this.gvResult.Columns[7].FooterStyle.CssClass = "hidden";
        this.gvResult_RowDataBound();
    }
